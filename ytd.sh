...
# Base download command with retry delay
command='--no-mtime -o /storage/emulated/0/Pictures/YouTubeVideos/%(title)s.%(ext)s --restrict-filenames --retry-sleep 5'
...
if [[ $? -eq 0 ]]; then
    ...
    echo -e "\e[031mDownload failed. This may be due to too many requests (HTTP 429). Please wait a few minutes and try again, or check your internet connection.\n$kill_color"
    echo -e "\e[031mIf the issue persists, ensure yt-dlp is updated: python -m pip install yt-dlp --user\n$kill_color"
fi
