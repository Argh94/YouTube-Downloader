...
if [[ ! -f "$COOKIE_FILE" ]]; then
    echo -e "\e[031mCookies file not found at $COOKIE_FILE. Please provide a cookies.txt file for YouTube authentication.\n$kill_color"
    echo -e "\e[031mSee https://github.com/yt-dlp/yt-dlp/wiki/FAQ#how-do-i-pass-cookies-to-yt-dlp for details.\n$kill_color"
    exit 1
fi
...
echo -e "\e[032m" "║ ♚ Author: You (https://Github.com/Argh94) ║"
echo -e "\e[036m" "╚════════════════════════════════════╝"
...
# Base download command with retry delay and cookies
command='--no-mtime -o /storage/emulated/0/Pictures/YouTubeVideos/%(title)s.%(ext)s --restrict-filenames --retry-sleep 5 --cookies '"$COOKIE_FILE"
...
if [[ $? -eq 0 ]]; then
    ...
    echo -e "\e[031mDownload failed. This may be due to too many requests (HTTP 429) or authentication issues.\n$kill_color"
    echo -e "\e[031mPlease wait 10-15 minutes and try again, or check your cookies file ($COOKIE_FILE).\n$kill_color"
