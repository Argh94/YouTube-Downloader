...
command -v ffmpeg &>/dev/null || {
    echo -e "\e[031mffmpeg not found. Installing ffmpeg...\n$kill_color"
...
if [[ ! -f "$COOKIE_FILE" ]]; then
    echo -e "\e[033mWarning: Cookie file ($COOKIE_FILE) not found. Some videos (e.g., Shorts) may require authentication.\n$kill_color"
    echo -e "\e[033mTo enable authentication, export your YouTube cookies to $COOKIE_FILE. See yt-dlp documentation for details.\n$kill_color"
fi
...
# Base download command with extended retry delay, retries, cookies, and merge output
command='--no-mtime -o /storage/emulated/0/Pictures/YouTubeVideos/%(title)s%(id)s.%(ext)s --restrict-filenames --retry-sleep 15 --retries 5 --force-ipv4 --merge-output-format mp4'
[[ -f "$COOKIE_FILE" ]] && command="$command --cookies $COOKIE_FILE"
...
if [[ $? -eq 0 ]]; then
    ...
    command -v termux-toast &>/dev/null && termux-toast "Download completed: /storage/emulated/0/Pictures/YouTubeVideos"
else
    ...
    echo -e "\e[031m- Authentication required: YouTube Shorts or restricted videos may need authentication. Try adding cookies to $COOKIE_FILE.\n$kill_color"
