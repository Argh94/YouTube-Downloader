...
available_space=$(df -m /storage/emulated/0 | tail -1 | awk '{print $4}')
if [[ $available_space -lt $required_space ]]; then
    echo -e "\e[031mInsufficient storage space! Need at least $required_space MB, but only $available_space MB available.\n$kill_color"
    exit 1
fi
...
if [[ $? -eq 0 ]]; then
    ...
    echo -e "\e[031m- Authentication required: Some videos (e.g., Shorts) may need authentication. Try a different video.\n$kill_color"
