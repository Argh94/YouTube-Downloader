...
# Base download command with extended retry delay
command='--no-mtime -o /storage/emulated/0/Pictures/YouTubeVideos/%(title)s.%(ext)s --restrict-filenames --retry-sleep 10 --force-ipv4'
...
if [[ $? -eq 0 ]]; then
    ...
    echo -e "\e[031m- Too many requests (HTTP 429): Wait 10-15 minutes and try again.\n$kill_color"
    echo -e "\e[031m- Authentication required: Some videos (e.g., Shorts) may need a cookies.txt file.\n$kill_color"
